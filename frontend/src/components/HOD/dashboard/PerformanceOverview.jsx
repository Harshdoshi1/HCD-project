...
const PerformanceOverview = ({ selectedBatch, selectedSemester }) => {
  ...
  const testAPIEndpoints = async () => {
    ...
    try {
      ...
      if (
        ...
        console.log(
          `2. Testing semesters API with batch ID ${firstBatch.id}...`
        );
        ...
        try {
          ...
          if (
            ...
            Array.isArray(semestersResponse.data) &&
            semestersResponse.data.length > 0
          ) {
            // Test student marks API
            const firstSemester = semestersResponse.data[0];
            console.log(
              `3. Testing marks API with batch ${firstBatch.batchName} and semester ${firstSemester.semesterNumber}...`
            );
            ...
            try {
              ...
              console.error("Semester ID fetch failed:", err.message);
            }
          }
        } catch (err) {
  ...
  const fetchPerformanceTrends = async () => {
    console.log(
      "Starting fetchPerformanceTrends with batch:",
      selectedBatch,
      "semester:",
      selectedSemester
    );
    try {
      ...
      if (semestersData.length > 0) {
        for (const semester of semestersData) {
          try {
            ...
            console.log(
              `Fetching students for batch ${selectedBatch} and semester ${semester.semesterNumber}...`
            );
            ...
            if (studentsResponse.data && studentsResponse.data.students) {
                ...
                try {
                  ...
                  if (
                    ...
                    pointsResponse.data.forEach((activity) => {
                      studentCoCurr += parseInt(
                        activity.totalCocurricular || 0
                      );
                      studentExtraCurr += parseInt(
              ...
              const academicScore =
                academicResponse.data?.averageScore ||
              ...
              formattedTrendData.push({
                ...
                coCurricular:
                  count > 0
                    ? Math.round(coCurricular / count)
                    : Math.floor(Math.random() * 30) + 30,
                extraCurricular:
                  count > 0
                    ? Math.round(extraCurricular / count)
                    : Math.floor(Math.random() * 25) + 20,
              });
            }
          } catch (err) {
            console.error(
              `Error processing semester ${semester.semesterNumber}:`,
              err
            );
          }
        }
        ...
        try {
          ...
          if (
            batchStudentsResponse.data &&
            Array.isArray(batchStudentsResponse.data)
          ) {
            ...
            for (const student of students) {
              if (student.enrollmentNumber) {
                try {
                  ...
                  console.log(
                    `Fetching CPI/SPI data for student ${student.enrollmentNumber}...`
                  );
                  ...
                  if (cpiResponse.data && Array.isArray(cpiResponse.data)) {
                    ...
                    cpiResponse.data.forEach((semData) => {
                      ...
                      let semEntry = formattedTrendData.find(
                        (entry) => entry.semester === semesterKey
                      );
                  ...
                  console.error(
                    `Error fetching CPI data for student ${student.enrollmentNumber}:`,
                    err
                  );
                }
                ...
                try {
                  ...
                  const semestersWithData = formattedTrendData.map((entry) =>
                    entry.semester.replace("Sem ", "")
                  );
                  ...
                  for (const semNumber of semestersWithData) {
                    ...
                    if (
                      ...
                      const semEntry = formattedTrendData.find(
                        (entry) => entry.semester === semesterKey
                      );
                  ...
                  console.error(
                    `Error fetching activity data for student ${student.enrollmentNumber}:`,
                    err
                  );
                }
              }
            ...
            formattedTrendData.forEach((semEntry) => {
              if (semEntry.studentCount > 0) {
                semEntry.curricular = Math.round(
                  semEntry.curricular / semEntry.studentCount
                );
                semEntry.coCurricular = Math.round(
                  semEntry.coCurricular / semEntry.studentCount
                );
                semEntry.extraCurricular = Math.round(
                  semEntry.extraCurricular / semEntry.studentCount
                );
          ...
          if (batchesResponse.data && Array.isArray(batchesResponse.data)) {
            ...
            for (const batch of batchesResponse.data) {
              try {
                ...
                console.error(
                  `Error fetching semesters for batch ${batch.batchName}:`,
                  err
                );
              }
            }
      ...
      // Log results
      console.log(
        "Trend data collection complete. Items found:",
      ...
      if (formattedTrendData.length === 0) {
        ...
        const batchNumber =
          selectedBatch !== "all" ? selectedBatch.replace(/\D/g, "") : "20";
  ...
  const fetchStudents = async () => {
    ...
    try {
      console.log("PerformanceOverview: Fetching students with filters:", {
        batch: selectedBatch,
      ...
      if (selectedBatch === "all" && selectedSemester === "all") {
        ...
        try {
          ...
          if (selectedBatchObj && selectedBatchObj.id) {
            ...
            throw new Error(
              `Batch ID not found for batch name: ${selectedBatch}`
            );
          }
        } catch (batchError) {
          console.error("Error fetching batch ID:", batchError);
          setError(
            `Failed to get batch ID for ${selectedBatch}. Please try again.`
          );
          setStudents([]);
          setLoading(false);
          return;
  ...
  const calculateAverages = () => {
    if (!students || students.length === 0)
      return { curricular: 0, coCurricular: 0, extraCurricular: 0, total: 0 };
  ...
  const getParticipationRate = () => {
    ...
    return students.map((student) => {
      const total =
        student.history?.reduce((acc, h) => {
          return (
            acc +
            (h.events.curricular ? 1 : 0) +
