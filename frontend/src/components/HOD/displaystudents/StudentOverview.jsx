...
const StudentOverview = ({ student }) => {
    ...
    useEffect(() => {
        ...
        if (student && student.enrollmentNumber) {
            ...
            console.log('begining Fetching student academic data for enrollment number:', student.enrollmentNumber);
        }
    }, [student]);
    ...
    const fetchStudentAcademicData = async (enrollmentNumber) => {
        try {
            ...
            if (data && data.length > 0) {
                ...
                // Format SPI data for chart
                const spiData = sortedData.map(item => ({
                    semester: item.Semester?.semesterNumber,
                    value: parseFloat(item.SPI).toFixed(2)
                }));
                ...
                // Get current CPI (from the latest semester)
                const currentCPI = sortedData.length > 0
                    ? parseFloat(sortedData[sortedData.length - 1].CPI).toFixed(2)
                    : null;
                ...
                <div className="overview-card-sdp-content">
                    {academicData.loading ? (
                        ...
                        <>
                            ...
                            <div className="gpa-chart">
                                <h4 className="gpa-chart-title">SPI Progression</h4>
                                <div className="gpa-bar-container">
                                    {academicData.spiData?.length > 0 ? (
                                        academicData.spiData.map((semGpa) => (
                                            <div key={semGpa.semester} className="gpa-bar-item">
                                                <div className="gpa-bar spi-bar" style={{ height: `${(semGpa.value / 10) * 100}%` }}>
                                                    <span className="gpa-value">{semGpa.value}</span>
                                                </div>
                                                <div className="semester-label">Sem {semGpa.semester}</div>
                                            </div>
                                        ...
                                        <div className="no-data-message">No SPI data available</div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="cumulative-gpa">
                                <div className="cumulative-value">
                                    <span className="cumulative-label">Current CPI</span>
                                    <span className="cumulative-number">
                                        {academicData.currentCPI || 'N/A'}
                                    </span>
                                    {academicData.currentCPI && (
                                        <div className="cgpa-progress">
                                            <div
                                                className="cgpa-fill"
                                                style={{ width: `${(academicData.currentCPI / 10) * 100}%` }}
                                            ></div>
                                        </div>
                                    )}
                                </div>
